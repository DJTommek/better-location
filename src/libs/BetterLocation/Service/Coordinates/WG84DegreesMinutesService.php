	...
	public static function findInText($text): array {
		...
		if (preg_match_all('/' . self::getRegex() . '/', $text, $matches)) {
			...
			for ($i = 0; $i < count($matches[0]); $i++) {
				...
				if ($switch) {
					list($latDegrees, $latMinutes) = explode('°', $matches[5][$i]);
					list($lonDegrees, $lonMinutes) = explode('°', $matches[2][$i]);
					$temp = $latHemisphere;
					$latHemisphere = $lonHemisphere;
					$lonHemisphere = $temp;
				} else {
					list($latDegrees, $latMinutes) = explode('°', $matches[2][$i]);
					list($lonDegrees, $lonMinutes) = explode('°', $matches[5][$i]);
				}
				...
				$results[] = new BetterLocation(
					Coordinates::wgs84DegreesMinutesToDecimal(floatval($latDegrees), floatval($latMinutes), $latHemisphere),
					Coordinates::wgs84DegreesMinutesToDecimal(floatval($lonDegrees), floatval($lonMinutes), $lonHemisphere),
					sprintf(self::NAME),
				);
			}
		}

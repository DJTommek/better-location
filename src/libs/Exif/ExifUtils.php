...
class ExifUtils
{
	...
	{
		$degrees = count($exifCoord) > 0 ? self::floatConvert($exifCoord[0]) : 0;
		$minutes = count($exifCoord) > 1 ? self::floatConvert($exifCoord[1]) : 0;
		$seconds = count($exifCoord) > 2 ? self::floatConvert($exifCoord[2]) : 0;

		return self::flip($hemisphere) * ($degrees + $minutes / 60 + $seconds / 3600);
	}

	/**
	 * Convert float written as string into actual float
	 *
	 * @see https://en.wikipedia.org/wiki/Geotagging
	 * @example floatConvert('5683/100') === 56.83
	 */
	public static function floatConvert(string $coordPart): float
	{

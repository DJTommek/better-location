...
class HelpCommand extends Command
{
	public function __construct($update, $tgLog, $loop) {
		...
		$text .= sprintf('Example if you send coordinates <code>%1$f,%2$f</code> or link https://www.waze.com/ul?ll=%1$f,%2$f', $lat, $lon) . PHP_EOL;
		$text .= PHP_EOL;
		$text .= $result;
		...
		$text .= sprintf(Icons::INFO . ' Note: Bot is currently in development. Check source code on Github <a href="%1$s%2$s">%2$s</a> for more info.', 'https://github.com/', 'DJTommek/better-location') . PHP_EOL;
		$this->reply($text);
	}
}